    ...
    reducers: {
        addProduct: (state, {payload}) => {
          ...
          if (index !== -1) {
              ...
              state.carts.push({ ...payload, cartQuantity: 1 }); 
          }
          // const newCart = [...state.carts]
          // newCart.pust(payload)
          // return {
          //   ...state,
          //   carts: newCart
          // }
        },
        handleCartQuantity: (state, {payload}) => {
            const { id, quantityChange } = payload;
            const productIndex = state.carts.findIndex(item => item.id === id);

            if (productIndex !== -1) {
                // Nếu sản phẩm có trong giỏ hàng
                const newQuantity = state.carts[productIndex].cartQuantity + quantityChange || 1;
                state.carts[productIndex].cartQuantity = newQuantity;
            }
        },
        deleteProduct: (state, {payload}) => {
            state.carts = state.carts.filter((val) => val.id != payload)
        },
    }
})
export const {reducer: BtShoeShopReducer, actions: BtShoeShopActions} = slice